...
async def list_service_orders(
    ...
    - Supervisor: vê O.S. das lojas sob sua supervisão
    - Operator: vê O.S. da sua loja
    """
...
async def export_fechamento(
    ...
    content = export.generate_fechamento_excel(
        orders,
        store_name,
        date_from,
        date_to,
...
async def export_fechamento_resumo(
    ...
    content = export.generate_resumo_fechamento_excel(orders, store_name, date_from, date_to)
...
async def export_fechamento_secao(
    ...
    content = export.generate_section_fechamento_excel(
        orders, dept_key, store_name, date_from, date_to
    )
...
async def export_conferencia(
    ...
    if conference_statuses:
        ...
        query = query.where(SOModel.status == OSStatus.DUPLICATE.value)
    ...
    if date_from is not None:
        ...
        query = query.where(
            or_(
                and_(SOModel.service_date.is_(None), SOModel.entry_time >= date_from),
                SOModel.service_date >= date_from.date(),
            )
        )
    ...
    if date_to is not None:
        ...
        query = query.where(
            or_(
                and_(SOModel.service_date.is_(None), SOModel.entry_time <= date_to_eod),
                SOModel.service_date <= date_to.date(),
            )
        )
    ...
    if plate is not None:
        ...
        query = query.where(
            or_(
                SOModel.vehicle_plate.ilike(f"%{plate}%"),
                SOModel.external_os_number.ilike(f"%{plate}%"),
            )
        )
    ...
    if worker_id is not None:
        ...
        query = query.where(
            exists().where(
                ServiceOrderWorker.service_order_id == SOModel.id,
    ...
    if flag:
        ...
        flag_conditions = []
        if "courtesy" in flag:
            flag_conditions.append(SOModel.is_courtesy.is_(True))
        if "galpon" in flag:
            flag_conditions.append(SOModel.is_galpon.is_(True))
        if "retorno" in flag:
            flag_conditions.append(SOModel.is_return.is_(True))
        if flag_conditions:
...
async def finalize_service_order(
    ...
    result = await service.finalize_service_order(db, service_order_id, data, current_user)
