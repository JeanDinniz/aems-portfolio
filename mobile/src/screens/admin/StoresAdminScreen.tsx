...
import { useStoresAdmin, useToggleStoreActive } from '@/hooks/useStoresAdmin';
...
/**
 * Lojas (Admin — Fatia 5a). Lista completa (o backend devolve todas) com busca
 * client-side, badge Ativo/Inativo e ação Ativar/Desativar inline (gated por
 * `useCanEdit('stores')`). Não editamos galpão nem estoque compartilhado aqui.
 */
export function StoresAdminScreen({ navigation }: AdminStackScreenProps<'StoresAdmin'>) {
    const { colors } = useTheme();
    const canEdit = useCanEdit('stores');

    const [search, setSearch] = useState('');
    const { data, isLoading, isError, refetch, isRefetching } = useStoresAdmin();
    const toggle = useToggleStoreActive();
    ...
    return (
        <View className="flex-1 bg-neutral-50 dark:bg-dark-bg">
            <ScreenHeader
                title="Lojas"
                subtitle={isLoading ? 'Carregando...' : `${stores.length} lojas`}
            ...
            <View className="flex-1 pt-3">
                {isLoading ? (
                    ...
                    <EmptyState
                        ...
                        description={search ? 'Ajuste a busca.' : 'Ainda não há lojas cadastradas.'}
                    />
                ) : (
                    <FlashList
                        data={stores}
