...
export function FilmRollSelector({
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filtrar por tonalidade, mas nunca remover a bobina já selecionada
      if (!tonality) return all
      return all.filter((r) => r.tonality === tonality || (includeRollIds?.includes(r.id) ?? false))
    },
