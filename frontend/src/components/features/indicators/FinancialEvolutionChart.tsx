import { memo } from 'react';
import {
  Bar,
  Line,
  ComposedChart,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  Legend,
  ResponsiveContainer,
} from 'recharts';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Tabs, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { useIsDark } from '@/hooks/useIsDark';
import { CHART_COLORS } from '@/constants/chartColors';
import { formatCompactCurrency, formatCurrency } from '@/lib/utils';
import { formatDateTick } from '@/components/features/dashboard/chartAxis';
import type {
  FinancialEvolutionGranularity,
  FinancialEvolutionPoint,
} from '@/types/indicators.types';

interface FinancialEvolutionChartProps {
  data: FinancialEvolutionPoint[] | undefined;
  granularity: FinancialEvolutionGranularity;
  onGranularityChange: (value: FinancialEvolutionGranularity) => void;
}

const EMPTY_STATE = (
  <div className="flex items-center justify-center h-48 text-gray-500 text-sm">
    Sem dados para o período
  </div>
);

export const FinancialEvolutionChart = memo(function FinancialEvolutionChart({
  data,
  granularity,
  onGranularityChange,
}: FinancialEvolutionChartProps) {
  const isDark = useIsDark();
  const chartData = data ?? [];

  return (
    <Card className="bg-white dark:bg-[#161616] border-gray-200 dark:border-[#1E1E1E]">
      <CardHeader className="pb-2">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <CardTitle className="text-sm font-semibold text-gray-700 dark:text-gray-300">
              Evolução Financeira
            </CardTitle>
            <p className="text-xs text-gray-500 dark:text-gray-500 mt-0.5">
              Faturamento · Custo · Margem
            </p>
          </div>
          <Tabs
            value={granularity}
            onValueChange={(v) => onGranularityChange(v as FinancialEvolutionGranularity)}
          >
            <TabsList className="h-8 bg-gray-100 dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#2a2a2a]">
              <TabsTrigger value="day" className="text-xs h-6 px-3">
                Diário
              </TabsTrigger>
              <TabsTrigger value="week" className="text-xs h-6 px-3">
                Semanal
              </TabsTrigger>
              <TabsTrigger value="month" className="text-xs h-6 px-3">
                Mensal
              </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      </CardHeader>
      <CardContent>
        {chartData.length === 0 ? (
          EMPTY_STATE
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={chartData} margin={{ top: 8, right: 16, left: 8, bottom: 4 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#2a2a2a' : '#e5e7eb'} />
              <XAxis
                dataKey="period"
                tickFormatter={(v) => formatDateTick(v as string, granularity)}
                tick={{ fontSize: 11, fill: isDark ? '#6b7280' : '#4b5563' }}
                axisLine={false}
                tickLine={false}
              />
              <YAxis
                yAxisId="left"
                tick={{ fontSize: 11, fill: isDark ? '#6b7280' : '#4b5563' }}
                tickFormatter={(v) => formatCompactCurrency(Number(v))}
                axisLine={false}
                tickLine={false}
              />
              <YAxis
                yAxisId="right"
                orientation="right"
                tick={{ fontSize: 11, fill: isDark ? '#6b7280' : '#4b5563' }}
                tickFormatter={(v) => `${v}%`}
                axisLine={false}
                tickLine={false}
              />
              <Tooltip
                contentStyle={{
                  backgroundColor: isDark ? '#1a1a1a' : '#ffffff',
                  border: isDark ? '1px solid #2a2a2a' : '1px solid #e5e7eb',
                  borderRadius: '6px',
                  color: isDark ? '#e5e5e5' : '#111111',
                  fontSize: '12px',
                }}
                labelFormatter={(v) => formatDateTick(v as string, granularity)}
                formatter={(value, name) => {
                  if (name === 'Margem %') {
                    return [`${(value as number).toFixed(1)}%`, name as string];
                  }
                  return [formatCurrency(value as number), name as string];
                }}
              />
              <Legend
                verticalAlign="top"
                align="right"
                wrapperStyle={{ fontSize: '12px', color: isDark ? '#888' : '#4b5563' }}
              />
              <Bar
                yAxisId="left"
                dataKey="revenue"
                name="Faturamento"
                fill={CHART_COLORS.brand}
                radius={[4, 4, 0, 0]}
                maxBarSize={32}
              />
              <Line
                yAxisId="left"
                type="monotone"
                dataKey="cost"
                name="Custo"
                stroke={CHART_COLORS.red}
                strokeWidth={2}
                dot={false}
                activeDot={{ r: 4 }}
              />
              <Line
                yAxisId="right"
                type="monotone"
                dataKey="margin_pct"
                name="Margem %"
                stroke={CHART_COLORS.green}
                strokeWidth={2}
                strokeDasharray="5 4"
                dot={false}
                activeDot={{ r: 4 }}
              />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      </CardContent>
    </Card>
  );
});
